...
cat("绘制数据处理完毕\n")
graph=ggplot()+
geom_point(aes(x=data_mapM$xp,y=data_mapM$Yscore,color=data_mapM$Yscore),shape="★",size=5)+
...
mx=data_mapM[mns,2]
my=data_mapM[mns,3]
graph=graph+geom_text(aes(x=mx,y=my),label=paste("↙",substr(mtext,1,15)),hjust=0,vjust=0,check_overlap = T)
}else{cat("不标注最大信息\n")}
...
# Core function
SWChen=function(x,y,Classer="rv",m=1,t=0,Larg=600,message="★SWChen★",norder=5){
address=getwd()
